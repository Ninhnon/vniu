...
const CartScreen = ({ navigation }: TabsStackScreenProps<'Cart'>) => {
  ...
  const handleOrder = () => {
    const itemsToOrder = cartItems.filter((item: { cartItemId: any }) => selectedItems.includes(item?.id))
  ...
  const renderCartItem = ({ item }:{item:any}) => {
    ...
    return (
      <View style={styles.cartItem}>
        ...
        />
        <Image source={{ uri: item?.productImages[0].imageUrl }} style={styles.productImage} />
        <View style={styles.productDetails}>
          <Text style={[styles.productName, { color: colors.text }]}>{item?.productName}</Text>
          <View style={styles.quantityContainer}>
            <Text style={[styles.productVariation, { color: colors.text }]}>Colour: {item?.colourName}</Text>
            <Text style={[styles.productVariation, { color: colors.text }]}>Size: {item?.sizeOptionName}</Text>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Text style={[styles.quantity, { color: colors.text }]}>{item?.cartItem?.quantity}</Text>
              <TouchableOpacity
                style={styles.quantityButton}
          ...
          <Text style={[styles.productVariation, { color: colors.text }]}>$ {formatNumber(item?.salePrice)}</Text>
        </View>
      </View>
    )
  }
